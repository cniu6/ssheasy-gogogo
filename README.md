...
支持通过 URL 参数自动填充或直接连接：

```
http://localhost:8080/cl/connect?host=example.com&port=22&user=root&password=yourpassword
```
...
| 参数 | 说明 | 默认值 |
|------|------|--------|
| `host` | SSH 服务器主机名或 IP 地址 | 必填（当 connect=true 时） |
| `port` | SSH 服务器端口 | 22 |
| `user` | SSH 用户名 | 必填 |
| `password` | SSH 密码 | – |
| `pk` | SSH 私钥（字符串形式） | – |
...
**注意事项：**
- 当 `connect` 为 `"true"` 或未提供时，`host` 参数为必填项
- 当 `connect` 为 `"false"` 时，只会填充连接表单，不会自动连接
- 出于安全考虑，不建议在 URL 中包含密码，建议使用 SSH 密钥认证
